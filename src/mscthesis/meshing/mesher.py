...
SUBSTOMATAL_CAVITY_MARGIN_FRACTION = 0.15
TOLERANCE = 0.01
MESH_SCALE = 1.0
...
def iterative_affine_transformation(
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Script for automating .brep --> .msh conversion"
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Path to output .msh file. if not provided, will use input path with .msh extension",
    )
    p.add_argument(
        "--suppress", default=False, action="store_true", help="Suppress verbose output"
    )
    p.add_argument(
        "--suppress-gmsh",
        default=True,
    ...
    p.add_argument(
        "--bm",
        type=float,
        default=BOUNDARY_MARGIN_FRACTION,
        help=f"Boundary margin fraction for cylinder plug no-flux boundaries (default: {BOUNDARY_MARGIN_FRACTION:.2f})",
    )
    p.add_argument(
        "--scm",
        type=float,
        default=SUBSTOMATAL_CAVITY_MARGIN_FRACTION,
        help=f"Substomatal cavity margin fraction for cylinder plug (default: {SUBSTOMATAL_CAVITY_MARGIN_FRACTION:.2f})",
    )
    p.add_argument(
        "--tolerance",
        type=float,
        default=TOLERANCE,
        help=f"Tolerance for geometric comparisons (default: {TOLERANCE:.3f})",
    )
    p.add_argument(
        "--mesh-scale",
        type=float,
        default=MESH_SCALE,
        help=f"Global mesh scale factor (default: {MESH_SCALE:.2f})",
    )
    p.add_argument(
        "--min-resolution",
        type=float,
        default=MINIMUM_RESOLUTION,
        help=f"Minimum mesh resolution (default: {MINIMUM_RESOLUTION:.3f})",
    )
    p.add_argument(
        "--max-resolution",
        type=float,
        default=MAXIMUM_RESOLUTION,
        help=f"Maximum mesh resolution (default: {MAXIMUM_RESOLUTION:.3f})",
    )
    p.add_argument(
        "--min-distance",
        type=float,
        default=MINIMUM_DISTANCE,
        help=f"Minimum distance for mesh size field (default: {MINIMUM_DISTANCE:.3f})",
    )
    p.add_argument(
        "--max-distance",
        type=float,
        default=MAXIMUM_DISTANCE,
        help=f"Maximum distance for mesh size field (default: {MAXIMUM_DISTANCE:.3f})",
    )
    p.add_argument(
        "--inlet-base-resolution-factor",
        type=float,
        default=INLET_BASE_RESOLUTION_FACTOR,
        help=f"Base resolution factor for inlet (default: {INLET_BASE_RESOLUTION_FACTOR:.2f})",
    )
    p.add_argument(
        "--plot",
        default=False,
        action="store_true",
        help="Open the Gmsh GUI to visualize the mesh after generation",
    )
    ...
    )  # fix getBoundingBox to use STL bounds (more robust)
    ...
    reporter.print(
        f"Identified maximum radial distance in xy-plane: {max_distance:.3f}"
    )
    ...
    reporter.print(
        f"Created airspace by boolean cutting cylinder from tissue, resulting in # {len(volumes)} volume(s)"
    )
    largest_volume = 0
    ...
    def transformation(
        ...
        return [
            ...
            0,
            0,
            -center[0] * (target_size / size[2]),
            0,
            (target_size / size[2]),
            0,
            -center[1] * (target_size / size[2]),
            0,
            0,
            (target_size / size[2]),
            -(center[2] - size[2] / 2) * (target_size / size[2]),
            0,
            0,
            0,
            1,
    ...
    iterations = iterative_affine_transformation(
        airspace, transformation, error, max_iterations=5, target_size=1.0
    )

    if not args.suppress:
        center, size = get_bbox(airspace)
        reporter.print(
            f"Applied {iterations} affine transformation(s) to airspace. New center: {center}, New size: {size}"
        )
    ...
    def iscurved(tag):
        ...
        if trigger:
            ...
            reporter.print(
                f"Curved area found, relative error: {area/curved_area_target - 1:.6f}"
            )
        return trigger
    ...
    gmsh.model.mesh.field.setNumbers(
        mesophyll_distance, "FacesList", mesophyll_surface_tags
    )
    ...
    gmsh.model.mesh.field.setNumbers(
        inlet_distance, "FacesList", [bottom_area_tag, top_area_tag]
    )
