...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Generate a simple swiss cheese mesh of non-overlapping spheres in .npy format"
    )
    p.add_argument(
        "output_path",
        type=str,
        default="test.npy",
        help="Output path for the generated geometry (.npy format)",
    )
    p.add_argument(
        "--seed",
        type=int,
        default=SEED,
        help=f"Random seed for sphere placement (default: {SEED})",
    )
    p.add_argument(
        "--resolution",
        type=int,
        default=RESOLUTION,
        help=f"Resolution of the output grid (default: {RESOLUTION})",
    )
    p.add_argument(
        "--n-spheres",
        type=int,
        default=N_SPHERES,
        help=f"Number of spheres to generate (default: {N_SPHERES})",
    )
    p.add_argument(
        "--min-radius",
        type=float,
        default=MIN_RADIUS,
        help=f"Minimum radius of spheres (default: {MIN_RADIUS})",
    )
    p.add_argument(
        "--max-radius",
        type=float,
        default=MAX_RADIUS,
        help=f"Maximum radius of spheres (default: {MAX_RADIUS})",
    )
    p.add_argument(
        "--depth",
        type=float,
        default=DEPTH,
        help=f"Depth of plug relative to plug radius of 1.0 (default: {DEPTH})",
    )
    p.add_argument(
        "--allowed-attempts",
        type=int,
        default=ALLOWED_ATTEMPTS,
        help=f"Maximum number of attempts to place spheres (default: {ALLOWED_ATTEMPTS})",
    )
    p.add_argument(
        "--min-spacing",
        type=float,
        default=MIN_SPACING,
        help=f"Minimum spacing between spheres (default: {MIN_SPACING})",
    )
    p.add_argument(
        "--suppress", default=False, action="store_true", help="Suppress console output"
    )
    p.add_argument(
        "--plot",
        default=False,
        action="store_true",
        help="Plot the generated geometry with open3d",
    )
    ...
    reporter.print(
        f"Generating swiss cheese geometry with approximately {args.n_spheres} spheres...  "
    )
    ...
    voxels = np.zeros(
        (args.resolution, args.resolution, args.resolution), dtype=np.uint8
    )
    centers = []
    radii = []
    ...
    for _ in range(args.n_spheres):
        ...
        while attempts < args.allowed_attempts:
            ...
            if (
                all(
                    np.linalg.norm(center - cen) > (radius + rad + args.min_spacing)
                    for cen, rad in zip(centers, radii)
                )
                and np.linalg.norm(center[:2]) <= max_xy
            ):
    ...
    if len(centers) < args.n_spheres:
        reporter.print(
            f"WARNING: Only packed {len(centers)} spheres out of {args.n_spheres} after {args.allowed_attempts} attempts per sphere. Consider increasing --allowed-attempts or reducing --n-spheres."
        )
    else:
