    ...
    #
        ...
        if args.groups:
            ...
            mesh, cell_tags, facet_tags = gmshio.read_from_msh(
                args.input_path, MPI.COMM_WORLD, rank, gdim=3
            )
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            for zc in zcenters:
                slices = grid.slice(
                    normal="z", origin=(0, 0, zc * (zmax - zmin) + zmin)
                )
                p.add_mesh(slices, scalars="uh", cmap="magma", clim=[0.00, 1.00])
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument(
        "input_path", type=str, help="Path to input mesh file (.npy, .stl, .msh, .bp)"
    )
    p.add_argument(
        "--groups",
        default=False,
        action="store_true",
        help="If input is a .msh file, plot physical groups",
    )
